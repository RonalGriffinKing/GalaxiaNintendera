<template>
  <div class="login-page">
    ...
    <form @submit.prevent="submit" class="login-card">
      ...
      >
        <i :class="isRegister ? 'fas fa-user-plus' : 'fas fa-right-to-bracket'"></i>
        <span>{{ loading ? (isRegister ? 'Creando cuenta...' : 'Iniciando sesion...') : (isRegister ? 'Crear cuenta' : 'Entrar') }}</span>
      </button>

      <button type="button" class="mode-link" @click="toggleMode">
        <i :class="isRegister ? 'fas fa-user-check' : 'fas fa-user-plus'"></i>
...
.auth-submit-btn {
  ...
  background:
    ...
    linear-gradient(135deg, #7c3aed 0%, #ec4899 58%, #f97316 100%);
  border: 1px solid rgba(255, 255, 255, 0.3);
  border-radius: 18px;
  box-shadow: 0 18px 34px rgba(147, 51, 234, 0.26), inset 0 1px 0 rgba(255, 255, 255, 0.26);
  color: #ffffff;
  display: inline-flex;
  font-size: 15px;
  font-weight: 950;
...
.auth-submit-btn:focus-visible:not(:disabled) {
  box-shadow: 0 22px 42px rgba(236, 72, 153, 0.34), 0 0 0 4px rgba(168, 85, 247, 0.16);
  filter: saturate(1.08);
  transform: translateY(-2px);
}

.auth-submit-btn:hover:not(:disabled)::after,
.auth-submit-btn:focus-visible:not(:disabled)::after {
  left: 110%;
}
