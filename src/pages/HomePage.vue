...
const latestCarousel = useTimedCarousel(latestPageCount, HOME_LATEST_SLIDE_DURATION)
const latestPage = latestCarousel.index
const latestProgressPercent = latestCarousel.progressPercent
const maxLatestPage = latestCarousel.maxIndex
...
const previousLatestPage = () => {
  latestCarousel.previous()
}

const selectLatestPage = (index) => {
  latestCarousel.select(index)
}
    ...
    <div class="home-page-content">
        ...
        <div v-else class="latest-empty">
          ...
          <HomeDiscoveryFeed
            ...
            :posts="featuredDashboardPosts"
            :threads="successfulThreads"
            :card-title="cardTitle"
            :card-action-icon="cardActionIcon"
            :format-ago="formatAgo"
            @open-community="(community) => community ? goCommunity(community) : router.push('/comunidad')"
            @open-archive="goLatestArchive"
            @open-post="goPost"
          />
        </main>
    </div>
...
@media (min-width: 1180px) and (max-width: 1450px) {
  .home-page {
    padding: var(--public-page-top, 82px) 14px 30px;
  }
  ...
  .home-shell :deep(.media-hero-card) {
    border-radius: 18px;
    padding: 14px;
  }

  .home-shell :deep(.media-copy h3) {
    font-size: clamp(20px, 2vw, 26px);
    margin-top: 10px;
  }

  .home-shell :deep(.media-copy p) {
    font-size: 12px;
    line-height: 1.38;
    margin-top: 8px;
    -webkit-line-clamp: 2;
  ...
  .home-shell :deep(.media-actions) {
    margin-top: 12px;
  }

  .home-shell :deep(.media-actions button),
  .home-shell :deep(.media-actions a) {
    min-height: 36px;
    padding: 8px 14px;
  }
}
...
@media (min-width: 1180px) and (max-height: 850px) {
  ...
  .home-hero {
    min-height: clamp(340px, 43vh, 430px);
  }
